...
7. 截图生成 PNG，发送到 QQ 群；英文原题正文不会出现在图片中，原题中的图表会作为独立图片节点附在题目卡中。
...
原题缓存仍用于生成中文摘要和供 AI 判断；其中的非公式图片会单独附在群聊题目卡中。
...
发到群里的主图只包含翻译后的中文题意、中文样例标签和其中保留的 LaTeX 公式，不直接渲染英文原题 HTML。原题中的图表会作为独立图片节点紧跟在主图后面。
